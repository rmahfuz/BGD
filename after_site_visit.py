...
NUM_SIM = 20 #number of simulations to run (from t = 0 to t = NUM_SIM-1)
...
class Agent:
    #Constructor:
    def __init__(self, data): #data is expected to be [row_x, row_y], each with CARDINALITY number of columns
         self.data_p = np.zeros((2,CARDINALITY))
         self.data_p[:,:] = data
         #print(data)
         #self.init_est = (sum(self.data_p[1,:])/sum(self.data_p[0,:])) #change
         self.init_est = 1
    ...
    def calc_gradient(self,b):
        ...
        for i in range(0,CARDINALITY):
            sum += (self.data_p[1,i] - (b*self.data_p[0,i]))*(-self.data_p[0,i])
...
def make_agent(seed):
    ...
    for j in range(0,CARDINALITY):
        rn.seed(seed+j)
        w = rn.sample(list(range(0,100)),1)#gaussian noise between 0 and 1 (CHANGE PLEASE)
...
agent1 = Agent(make_agent(0))
#agentList = [cp.copy(agent1) for agent1 in [0]*NUM_AGENTS] #declaring agent list
agentList = [agent1 for i in range(0,NUM_AGENTS)] #declaring agent list
...
for t in range(1,NUM_SIM):
    ...
    for j in range(0,NUM_AGENTS):
        if(j == 100):#making the 6th agent the selfish agent(6th if you consider the starting agent to be number 1)
        ...
        if ((j != 100) and abs(local_est[t,j] - local_est[t-1,j]) < THRESH):
            terminate = 1
            break
    if terminate == 1:
        break
